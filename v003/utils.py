...
def get_first_app(list_data):
    ...
    for i in list_data:
        if "wfTag" in i:
            wftag=i["wfTag"]
            wftag=wftag.replace("_workflow","")
            # wfTag存在
            if wftag is not None and wftag != "":
                # wfTag不为空，返回包含三个空字符串的列表
                
                if wftag not in res_list:
                    res_list.append(wftag)
...
if __name__ == "__main__":
    file_path = "/Users/bws/cursor_project_202506/git25/workflow_planning_06/v003/data/Stereo_Miner_Clustering_workflow.txt"
    data = read_json(file_path)
    first_appp = get_first_app(data["data"]["records"])
    print(first_appp)
    first_node = get_first_node(first_appp)
    print(first_node)
    omics = data["data"]["records"][0]["omics"]
    print(omics)
